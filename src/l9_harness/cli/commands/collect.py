...
def command(subject_path: Path, observation_dir: Path, output: Path) -> dict:
    ...
    return {'status': 'pass' if index['counts']['invalid'] == 0 else 'partial', 'artifacts': [{'path': output.as_posix()}], 'details': index['counts']}

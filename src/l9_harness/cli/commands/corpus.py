...
def command(action: str, remote: Path, cache: Path, outbox: Path) -> dict[str, Any]:
    if action == "pull":
        pull(remote, cache)
    elif action == "push":
        push(outbox, remote)
    ...
    return {
        ...
        "details": {
            ...
            "outbox": outbox.as_posix(),
            "remoteFiles": _count_files(remote),
            "cacheFiles": _count_files(cache),
            "outboxFiles": _count_files(outbox),
